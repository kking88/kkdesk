...
""",
        ...
        (
            ...
            "  build-RustDeskTempTopMostWindow:\n    if: ${{ contains(format(',{0},', inputs.build-targets), ',all,') || contains(format(',{0},', inputs.build-targets), ',windows,') }}\n    uses: ./.github/workflows/third-party-RustDeskTempTopMostWindow.yml",
            False,
        ),
        (
            ".github/workflows/flutter-build.yml",
        ...
        (
            ...
            "  build-rustdesk-ios:\n    if: ${{ inputs.upload-artifact && (contains(format(',{0},', inputs.build-targets), ',all,') || contains(format(',{0},', inputs.build-targets), ',ios,')) }}\n    name: build rustdesk ios ipa",
            False,
        ),
        (
            ".github/workflows/flutter-build.yml",
            "  build-for-macOS:\n    name: ${{ matrix.job.target }}",
            "  build-for-macOS:\n    if: ${{ contains(format(',{0},', inputs.build-targets), ',all,') || contains(format(',{0},', inputs.build-targets), ',macos,') }}\n    name: ${{ matrix.job.target }}",
            False,
        ),
        (
            ".github/workflows/flutter-build.yml",
            "  publish_unsigned:\n    needs:\n      - build-for-macOS\n      - build-for-windows-flutter\n      - build-for-windows-sciter\n    runs-on: ubuntu-latest\n    if: ${{ inputs.upload-artifact }}",
            "  publish_unsigned:\n    needs:\n      - build-for-macOS\n      - build-for-windows-flutter\n      - build-for-windows-sciter\n    runs-on: ubuntu-latest\n    if: ${{ inputs.upload-artifact && (contains(format(',{0},', inputs.build-targets), ',all,') || (contains(format(',{0},', inputs.build-targets), ',windows,') && contains(format(',{0},', inputs.build-targets), ',macos,'))) }}",
            False,
        ),
        (
            ".github/workflows/flutter-build.yml",
            "  build-rustdesk-android:\n    needs: [generate-bridge]\n    name: build rustdesk android apk ${{ matrix.job.target }}",
            "  build-rustdesk-android:\n    if: ${{ contains(format(',{0},', inputs.build-targets), ',all,') || contains(format(',{0},', inputs.build-targets), ',android,') }}\n    needs: [generate-bridge]\n    name: build rustdesk android apk ${{ matrix.job.target }}",
            False,
        ),
        (
            ".github/workflows/flutter-build.yml",
            "  build-rustdesk-android-universal:\n    needs: [build-rustdesk-android]\n    name: build rustdesk android universal apk\n    if: ${{ inputs.upload-artifact }}",
            "  build-rustdesk-android-universal:\n    needs: [build-rustdesk-android]\n    name: build rustdesk android universal apk\n    if: ${{ inputs.upload-artifact && (contains(format(',{0},', inputs.build-targets), ',all,') || contains(format(',{0},', inputs.build-targets), ',android,')) }}",
            False,
        ),
        (
            ".github/workflows/flutter-build.yml",
            "  build-rustdesk-linux:\n    needs: [generate-bridge]\n    name: build rustdesk linux ${{ matrix.job.target }}",
            "  build-rustdesk-linux:\n    if: ${{ contains(format(',{0},', inputs.build-targets), ',all,') || contains(format(',{0},', inputs.build-targets), ',linux,') }}\n    needs: [generate-bridge]\n    name: build rustdesk linux ${{ matrix.job.target }}",
            False,
        ),
        (
            ".github/workflows/flutter-build.yml",
            "  build-rustdesk-linux-sciter:\n    if: ${{ inputs.upload-artifact }}\n    runs-on: ${{ matrix.job.on }}",
            "  build-rustdesk-linux-sciter:\n    if: ${{ inputs.upload-artifact && (contains(format(',{0},', inputs.build-targets), ',all,') || contains(format(',{0},', inputs.build-targets), ',linux,')) }}\n    runs-on: ${{ matrix.job.on }}",
            False,
        ),
        (
            ".github/workflows/flutter-build.yml",
